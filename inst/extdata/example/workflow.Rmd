...
p <- plot_phu_yld_bms(ylds_phu_dmat, yield_obs)
```

```{r sc-step12-save, include = FALSE}
# save as JPEG
...
ggsave("figures/Fig10.png", plot = p,
       device = ragg::agg_png,
       width = 10, height = 7, units = "in",
       dpi = 300)
```

```{r sc-step12-choose, echo = TRUE}
# !!!USER DECISION STEP!!!
...
par_bnd <- tibble('lai_pot.pdb | change = relchg'  = c(-0.3, 0.3),
                  ...
                  'bm_e.pdb | change = relchg'     = c(-0.3, 0.3))
...
p <- plot_dotty_yields(yld_sim, yield_obs)
```

```{r sc-step2a1-save, include = FALSE}
ggsave("figures/Fig11.png", plot = p,
       device = ragg::agg_png,
       width = 10, height = 7, units = "in",
       dpi = 300)
```

```{r sc-step2a1-choose, echo = TRUE}
# !!!USER DECISION STEP!!!
# Fix the parameter changes you want to apply to the crops
crop_par_sel <- tibble(
  plant_name                       = c("corn", "cots", "pnut"),
  'bm_e.pdb | change = relchg'     = c(  -0.2,   -0.25,   0.3),
  'harv_idx.pdb | change = relchg' = c(  -0.15,  -0.25,   0.3),
  'lai_pot.pdb | change = relchg'  = c(  -0.2,   -0.25,   0.3),
...
p <- plot_phu_yld_bms(check_sim, yield_obs)
```

```{r sc-step2a3-save, include = FALSE}
ggsave("figures/Fig12.png", plot = p,
       device = ragg::agg_png,
       width = 10, height = 7, units = "in",
       dpi = 300)
```
...
p <- plot_esco_epco(wbal_sim, wyr_target, rel_wyr_limit = 0.05)
```

```{r sc-step2b1-save, include = FALSE}
ggsave("figures/Fig13.png", plot = p,
       device = ragg::agg_png,
       width = 10, height = 7, units = "in",
       dpi = 300)
```
...
par_check <- tibble('esco.hru | change = absval' = 0.09, # Adjust accordingly
                    'epco.hru | change = absval' = 0.25) # Adjust accordingly
...
```{r sc-step2b3-save, include = FALSE}
ggsave("figures/Fig14.png", plot = p,
       device = ragg::agg_png,
       width = 10, height = 7, units = "in",
       dpi = 300)
```
...
# Write selected esco and epco into hydrology.hyd --------------------------
# Option 1: Re-writing SWAT+ parameter file hydrology.hyd
hydrology_hyd <- SWATreadR::read_swat(paste0(model_path, '/hydrology.hyd'))

hydrology_hyd$esco <- 0.09 # set value
hydrology_hyd$epco <- 0.25 # set value if epco is considered
...
# n_combinations <- 5000
n_combinations <- 500 # For faster runs
...
parameter_boundaries <- tibble(
  ...
  'esco.hru | change = absval' = c(0.04, 0.12),
  'epco.hru | change = absval' = c(0.05, 0.12),
  'awc.sol | change = relchg' = c(-0.1, 0.1),
  #surface runoff
  'cn2.hru | change = relchg' = c(-0.17, -0.1),
  'cn3_swf.hru | change = absval' = c(0.55, 0.75), # Normalized range. Will be updated below
  # 'ovn.hru | change  = relchg ' = c(-0.25, 0.25),
  'surlag.hru | change = absval' = c(0.6, 2),
  # lateral flow (optional - use if lateral flow constitutes at least 5% of
  # total water yield)
  'lat_len.hru | change = abschg' = c(0, 20),
  'latq_co.hru | change = absval' = c(0.4, 0.7), # Normalized range. Will be updated below
  'bd.sol | change = relchg' = c(0.23, 0.28),
  'k.sol | change = relchg' = c(1, 1.75),
  ...
  # percolation/aquifer
  'perco.hru | change = absval' = c(0.3, 0.55), # Normalized range. Will be updated below
  ...
  # channel routing
  'chn.rte | change = absval' = c(0.08, 0.12),
  'chs.rte | change = relchg' = c(0, 0.35),
  'chk.rte | change = absval' = c(90, 100)
  # # sediment routing
...
p <- plot_parameter_identifiability(parameters = par_vals,
                               objectives = gof_sel,
                               run_fraction = .2)
p
```

```{r hc-step41-save, include = FALSE}
...
run_sel <- which(gof_sel$nse_q > 0.65 & abs(gof_sel$pb_q) < 10 & gof_sel$kge_q > 0.75 & gof_sel$p_30_100<0.15)
run_ids <- run_to_id(run_sel)
...
p <- plot_timeseries(flow_sim, flow_obs, run_ids = run_ids)
p
```

```{r hc-step43-save, include = FALSE}
ggsave(
  "figures/Fig17.png",
  plot = p,
  device = ragg::agg_png,
...
# be neccesary.
plot_dotty(par = par_vals, gof_all$kge_q,  y_label = "KGE value", n_col = 4, run_ids = run_ids)
plot_dotty(par = par_vals, gof_all$pb_q,  y_label = "PBIAS value", n_col = 4, run_ids = run_ids)
plot_dotty(par = par_vals, gof_all$p_30_100,  y_label = "P_30_100", n_col = 4, run_ids = run_ids)

p <- plot_dotty(par = par_vals, gof_all$nse_q,  y_label = "NSE value", n_col = 4, run_ids = run_ids)
p

# !!!USER DECISION STEP!!!
# If model performance results satisfy users, they should proceed to the validation step.
```

```{r hc-step41-interpret-save, include = FALSE}
ggsave(
  "figures/Fig16.png",
  plot = p,
  device = ragg::agg_png,
...
```{r final-plot-save, include = FALSE}
ggsave(
  "figures/Fig18.png",
  plot = p,
  device = ragg::agg_png,
